...
Why need it?
 1. 在資料倉儲中，選擇進行分析的資料量可能**極為龐大** 
 2. 「複雜的資料分析」與「探勘龐大的資料」會花費很長的時間
 
- 資料壓縮：希望獲得一個**比原始資料小的縮減資料集**，並且其**幾乎能保持原始資料的完整性**

- 策略：
    - 資料聚合：將資料進行聚合運算
      > [✍🏾](https://github.com/vanikk06/Machine-Learning/tree/master/Data#-%E8%B3%87%E6%96%99%E8%81%9A%E5%90%88-)
    - 子屬性集選擇：偵測並移除不相關、低相關、不重要的屬性
      > [✍🏽](https://github.com/vanikk06/Machine-Learning/tree/master/Data#-%E5%AD%90%E5%B1%AC%E6%80%A7%E9%9B%86%E9%81%B8%E6%93%87-)
    - 維度縮減：利用「編碼方式」來降低資料集大小
      > [✍🏼](https://github.com/vanikk06/Machine-Learning/tree/master/Data#-%E7%B6%AD%E5%BA%A6%E7%B8%AE%E6%B8%9B-)
    - 數值縮減：利用其他「較小資料表示法」來表示資料
      > E.g. 將資料以**模型**表示、以**分群**、**取樣**方式進行取代
      >> [✍🏿](https://github.com/vanikk06/Machine-Learning/tree/master/Data#-%E6%95%B8%E5%80%BC%E7%B8%AE%E6%B8%9B-)
    - 離散化或概念階層建立：將原始資料以較高層次的資料取代
      > [✍🏻](https://github.com/vanikk06/Machine-Learning/tree/master/Data#-%E8%B3%87%E6%96%99%E9%9B%A2%E6%95%A3%E5%8C%96-)
...
分析的資料集合可能包含數百個屬性，其中可能有多個屬性與探勘任務是無關或多餘的
> 篩選出主要屬性

- 屬性選擇：
    - 找到「較小的屬性集」，且此屬性集的結果要與使用全部屬性集的結果盡量接近
    - 降低屬性數目

- 啟發式
  > n 個屬性可能有 2<sup>n</sup> 數目的選擇
  >> 每個屬性，「要」與「不要」
    
  - 逐步向前選擇：一開始無任何屬性，先找一個最大的屬性，再找與其配對是「最有效益」的屬性
    > 少變多
  - 逐步向後刪除：一開始為全部屬性，再將影響「最容易被替代」的屬性剔除
    > 多變少
  - 向前選擇與向後刪除的組合
  - 決策樹歸納
...
通常是將一些舊有的屬性**結合**成一個新的屬性，以達到降低維度的目的
> 三維（立體） → 二維（平面）

- 優點
    - 可提升資料探勘的品質：會刪除掉一些無關的特徵或雜訊值
    - 易於視覺化呈現
    - 大幅降低演算法的時間及記憶體的消耗
- 方式：使用**編碼或轉換**來對原始資料進行縮減或壓縮
    - Lossless：無損壓縮，**原始資料重新建構**成一種壓縮的資料，且沒有遺失任何資料
    - Lossy：有損壓縮，僅能建立**接近原始資料**的壓縮資料
    

#### § 數值縮減 §

存取**明顯或影響較大**的資料即可，不須全部存取

- 模型
  > E.g. 將資料分佈分割成箱子，每個箱子存取資料的個數、平均或總合（如直方圖）
  
- 分群：根據**相似度**進行，並儲存群組資訊（E.g. 中心、直徑）
    - 效果：依據資料性質，若資料不乾淨則效果差
    - 可執行「階層式分群」，並將結果存成多維度樹狀架構
    - 有許多分群定義與演算法則可選擇
    
- 取樣：用較小的隨機樣本 n 表示原始資料 N
    - 選取代表資料的子集合
      > 當資料包含偏斜時，效能差

#### § 資料離散化 §

有些 分類演算法 僅能處理類別型資料的問題，因此必須將一些連續值進行離散化的動作\
再者，如果類別很多，但有些是不常發生的，就可以將部份類別進行結合，以降低類別個數

- 資料**離散化**：連續型（數值型） → 類別型
    - Step1：將連續值進行排序，再定義 n-1 個分割點，將資料分為 n 等份
    - Step2：將所有值**映射**至類別中

- 概念階層：可透過「較高層次」的概念取代「較低層次」的概念，來進行資料壓縮
  > E.g. 以「青年、中年、老年」取代「年齡屬性值」
